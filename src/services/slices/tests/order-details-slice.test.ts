...
const order: Order = {
  ...
  ingredients: ['643d69a5c3f7b9001cfa093c', '643d69a5c3f7b9001cfa093e', '643d69a5c3f7b9001cfa093e', '643d69a5c3f7b9001cfa093c'],
  status: 'done',
...
describe('ingredientDetailsSlice', () => {
  ...
  it('clearOrder', () => {
    ...
    const state = orderDetailsSlice.reducer({ ...initialState, data: order }, action);
    expect(state).toEqual(initialState);
  });
  ...
  describe('sendOrder', () => {
    ...
    it('fulfilled', () => {
      ...
      const state = orderDetailsSlice.reducer(initialState, action);
      expect(state).toEqual({ ...initialState, isLoading: false, isError: false, data: order });
    });

    it('rejected', () => {
      const action = { type: sendOrder.rejected.type };
      const state = orderDetailsSlice.reducer(initialState, action);
      expect(state).toEqual({ ...initialState, isLoading: false, isError: true, data: null });
    });
  });
